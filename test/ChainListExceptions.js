...
contract("ChainList", function(accounts){
  ...
  //Buying an article does not exist
  it("should throw an exception if you try to buy an article that does not exist", function(){
    return ChainList.deployed().then(function(instance){
      ...
      return chainListInstance.buyArticle(2, {from: seller, value: web3.utils.toWei(String(articlePrice), "ether")});
    }).then(assert.fail)
    .catch(function(error) {
      assert(true);
    }).then(function(){
      ...
      assert.equal(data[5], web3.utils.toWei(String(articlePrice), "ether"), "article price must be " + web3.utils.toWei(String(articlePrice)));
    })
  })
  ...
  it("should throw an exception if you try to buy your own article", function() {
    return ChainList.deployed().then(function(instance){
      ...
      return chainListInstance.buyArticle(1, {from: seller, value: web3.utils.toWei(String(articlePrice), "ether")});
    }).then(assert.fail)
    .catch(function(error){
      assert(true);
    }).then(function() {
      ...
      assert.equal(data[3], articleName, "article name must be " + articleName);
      assert.equal(data[4], articleDescription, "article description must be " + articleDescription);
      assert.equal(data[5], web3.utils.toWei(String(articlePrice), "ether"), "article price must be " + web3.utils.toWei(String(articlePrice), "ether"));
    });
  });
  ...
  it("should throw an exception if you try to buy an article for a value different from its price", function() {
    return ChainList.deployed().then(function(instance){
      chainListInstance = instance;
      return chainListInstance.buyArticle(1, {from: buyer, value: web3.utils.toWei(String(articlePrice + 1), "ether")});
    }).then(assert.fail)
    .catch(function(error){
      assert(true);
    }).then(function() {
  ...
  it("should throw an exception if you try to buy an article that has already been sold", function() {
    return ChainList.deployed().then(function(instance){
      chainListInstance = instance;
      return chainListInstance.buyArticle(1, {from: buyer, value: web3.utils.toWei(String(articlePrice), "ether")});
    }).then(function(){
      return chainListInstance.buyArticle(1, {from: web3.eth.accounts[0], value: web3.utils.toWei(String(articlePrice), "ether")});
    }).then(assert.fail)
    .catch(function(error){
      assert(true);
    }).then(function() {
